...
def test_generation_and_clustering_never_build_any_trajectory(monkeypatch):
    ...
    representatives = cluster_initial_candidate_points(generated.points)
...
def test_nonzero_reference_bias_changes_actual_endpoint_before_any_clustering():
    ...
    trajectory = build_representative_trajectories(cluster_initial_candidate_points(generated.points), (-5, 5))[0]
...
def test_clustering_uses_xy_even_when_directions_are_opposite():
    ...
    representatives = cluster_initial_candidate_points(points, position_radius_m=1.1)
    assert len(representatives) == 1
...
def test_clusters_have_bounded_diameter_and_no_single_link_chaining():
    points = generate_initial_candidate_points(
        _empty_scene(), (0, 0), [_sample("a", 1.0), _sample("b", 2.0), _sample("c", 3.0)]
    ).points
    representatives = cluster_initial_candidate_points(points, position_radius_m=1.1)
    assert [len(item.members) for item in representatives] == [2, 1]
    assert max(item.metadata["maximum_member_point_distance_m"] for item in representatives) <= 1.1
...
def test_representative_is_actual_medoid_and_sampling_multiplicity_does_not_inflate_solver_weight():
    ...
    representative = cluster_initial_candidate_points(points, position_radius_m=1.1)[0]
...
def test_clustering_partitions_source_peak_and_full_wall_sequence_not_ambiguous_joined_name():
    ...
    representatives = cluster_initial_candidate_points(points, position_radius_m=100)
...
def test_point_to_cluster_to_trajectory_recovers_known_geometry_and_shared_bias(backend, chunk_size):
    ...
    representatives = cluster_initial_candidate_points(generated.points, position_radius_m=0.1)
...
def test_duplicate_sources_mixed_references_and_out_of_range_reference_are_rejected():
    ...
    representatives = cluster_initial_candidate_points([point])
