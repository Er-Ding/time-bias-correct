...
```bash
cd /data/zhujun/differt_projects/time-bias-correct
./run_visualization_experiment.sh
```
...
│   │   │   ├── 03_peak_perturbations/# 扰动峰、漏检、未配对峰和配对结果
│   │   │   ├── 04_reverse_candidates/# 原始峰的全部反向候选轨迹
│   │   │   ├── 05_first_clustering/  # 第一次聚类代表及成员数
│   │   │   ├── 06_joint_solution/    # 原始峰的位置/偏差联合解及残差
│   │   │   ├── 07_perturbation_solutions/ # 每次 CSI 内部扰动的重复求解
│   │   │   └── 08_final_evaluation/  # 最终汇总位置、偏差及独立评估
│   │   └── repeat_001/...
│   └── UE002/...
└── summary/
...
06 展示原始峰的联合解，08 展示扰动汇总后的最终解，二者分别输出，不混用。
当前真实调用顺序是 02 得到原始峰，03 构建扰动观测；04–06 用原始峰求解，07 再对 03 的每次扰动观测重复反向追踪、聚类、求解，08 汇总。
椭圆为二维高斯名义 95% 范围，实际覆盖率尚未校准。

新运行的 bootstrap_diagnostics.json 额外保存各扰动的观测输入、原始候选、聚类代表、选中候选和残差；这是增加输出记录，不改变求解算法。
旧运行没有保存的内部候选不能事后恢复为原始执行证据，07 子目录会明确说明缺失。
现有定位流水线在成功结束后才整体发布诊断文件；旧失败运行没有可核验中间文件时，各步骤保留“缺失/失败”说明，不编造图。
