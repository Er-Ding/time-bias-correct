...
SOURCE_EXPERIMENT="${SOURCE_EXPERIMENT:-${PROJECT_ROOT}/outputs/point_clustering_experiment_20260909T022303_3991465}"
...
# DBSCAN：邻域距离（米）、核心点所需邻域点数（包含点自身）。
DBSCAN_EPS_M="${DBSCAN_EPS_M:-1.5}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_ROOT}/outputs/fine_dbscan_check_$(date -u +%Y%m%dT%H%M%S)_$$}"
# ===================================================================
...
exec "${PYTHON_BIN}" scripts/check_spectrum_workflow.py \
  ...
  --dbscan-eps-m "${DBSCAN_EPS_M}" --dbscan-min-samples "${DBSCAN_MIN_SAMPLES}" \
