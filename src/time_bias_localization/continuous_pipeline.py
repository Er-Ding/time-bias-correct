...
def build_continuous_problem(config, scene, nominal_peaks, source_indices, bs_position_m, bs_boresight_rad):
    ...
    observations = tuple(ContinuousObservation(
        ...
        angle_scale_rad=math.radians(settings["angle_scale_deg"]),
        length_scale_m=settings["length_scale_m"],
    ) for i, p in zip(source_indices, nominal_peaks, strict=True))
...
def localize_saved_music(config, *, scene_json, music_peaks_json, output_root,
    ...
    from .data import load_online_measurement_bytes
    ...
    with exclusive_output_root_lock(root):
        ...
        peak_rows = document.get("nominal", [])
        indices = document.get("nominal_source_indices", list(range(len(peak_rows))))
        if len(indices) != len(peak_rows):
            raise ValueError("冻结正式峰必须带有对应来源编号")
        peaks = [SimpleNamespace(aoa_rad=float(p["aoa_rad"]), delay_s=float(p["delay_s"])) for p in peak_rows]
